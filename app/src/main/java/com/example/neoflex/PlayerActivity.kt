...
import com.example.neoflex.databinding.ActivityPlayerBinding
import com.example.neoflex.databinding.BoosterBinding
import com.example.neoflex.databinding.MoreFeaturesBinding
import com.example.neoflex.databinding.SpeedDialogBinding
...
class PlayerActivity : AppCompatActivity(), AudioManager.OnAudioFocusChangeListener, GestureDetector.OnGestureListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        movieLinks = intent.getStringExtra("movieLink").toString()
...
//                }
    ...
    private fun createPlayer(){
        ...
        videoTitle.text = movieTitle
        videoTitle.isSelected = true
        player = ExoPlayer.Builder(this).setTrackSelector(trackSelector).build()
        doubleTapEnable()
//        val mediaItem = MediaItem.fromUri(playerList[position].artUri)
        val mediaItem = MediaItem.fromUri(movieLinks)
        Log.d("TAG", "createPlayer: "+ movieLinks)
...
//    }
    ...
    override fun onScroll(
        ...
        if(Math.abs(distanceX) < Math.abs(distanceY) && Math.abs(minSwipeY) > 50){
            if(e1.x < sWidth/2){
                //brightness
